  ...
  for (const event of events) {
    try {
      ...
      if (event.eventType !== 'EMAIL_SCHEDULED' || !payload.emailId) {
        await prisma.outboxEvent.update({ where: { id: event.id }, data: { status: 'FAILED', attempts: { increment: 1 }, lastError: 'unsupported outbox event' } });
        continue;
      }
      const email = await prisma.email.findUnique({ where: { id: payload.emailId } });
      if (!email || email.status === 'CANCELLED') {
        await prisma.outboxEvent.update({ where: { id: event.id }, data: { status: 'PUBLISHED', processedAt: new Date() } });
        continue;
      }
      await queue.add('send-email', { emailId: email.id, attempt: email.attemptCount }, {
...
async function processEmail(jobData: EmailJobData, jobId?: string): Promise<void> {
  ...
  try {
    ...
    const provider = createProvider(email.sender.provider);
    ...
    if (email.batchId) {
      await prisma.batch.update({ where: { id: email.batchId }, data: { processedEmails: { increment: 1 }, sentEmails: { increment: 1 } } });
      await finalizeBatchIfReady(email.batchId);
    }
    ...
    if (error instanceof RateLimitExceeded) {
      ...
      await queue.add('send-email', { emailId: email.id, attempt: jobData.attempt + 1 }, { delay: Math.max(0, error.retryAt.getTime() - Date.now()), jobId: getEmailJobId(email.id) });
      return;
    }
    ...
    if (transient) {
      await prisma.email.update({ where: { id: email.id }, data: { status: 'SCHEDULED', processingAt: null, lastError: message } });
      throw error;
    }

    await prisma.email.update({ where: { id: email.id }, data: { status: 'FAILED', failedAt: new Date(), lastError: message } });
    if (email.batchId) {
      await prisma.batch.update({ where: { id: email.batchId }, data: { processedEmails: { increment: 1 }, failedEmails: { increment: 1 } } });
      await finalizeBatchIfReady(email.batchId);
    }
...
async function finalizeBatchIfReady(batchId: string): Promise<void> {
  const batch = await prisma.batch.findUnique({ where: { id: batchId } });
  if (!batch || batch.processedEmails < batch.totalEmails) return;
  await prisma.batch.update({ where: { id: batchId }, data: { status: batch.failedEmails > 0 ? 'PARTIAL_FAILURE' : 'COMPLETED' } });
}

const worker = new Worker<EmailJobData>(EMAIL_QUEUE_NAME, async (job) => processEmail(job.data, job.id), {
  connection: redis as IORedis,
  concurrency: Number(process.env.WORKER_CONCURRENCY ?? 5),
  settings: { backoffStrategy: (attemptsMade) => Math.min(60000, 1000 * 2 ** attemptsMade + Math.floor(Math.random() * 500)) },
});
